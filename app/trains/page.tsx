...
import { Clock, Loader2, MapPin, ArrowRight } from 'lucide-react';
...
export default function TrainsPage() {
  ...
  // Helper to get formatted date (YYYY-MM-DD) for India timezone
  ...
  useEffect(() => {
    async function loadTrains() {
      ...
      const data = await fetchTrainsBetweenStations(fromStation, toStation, targetDate);
    ...
    <div className="min-h-screen px-4 py-6 md:p-12 max-w-md mx-auto pb-28">
      ...
      <div className="flex bg-surface-hover p-1 rounded-xl mb-3 border border-border-subtle relative">
        <button 
          ...
          className={`flex-1 py-2 rounded-lg text-[13px] font-semibold transition-all flex items-center justify-center gap-2 ${direction === 'BTA_PNBE' ? 'bg-surface border border-border-subtle shadow-sm text-white' : 'text-text-secondary'}`}
        >
          Bihta <ArrowRight size={14} className={direction === 'BTA_PNBE' ? 'text-accent' : ''} /> Patna
        </button>
        <button 
          onClick={() => setDirection('PNBE_BTA')} 
          className={`flex-1 py-2 rounded-lg text-[13px] font-semibold transition-all flex items-center justify-center gap-2 ${direction === 'PNBE_BTA' ? 'bg-surface border border-border-subtle shadow-sm text-white' : 'text-text-secondary'}`}
        >
      ...
      <div className="flex gap-2 mb-6">
        <button 
          ...
          className={`flex-1 py-1.5 rounded-lg text-[12px] font-bold tracking-wider uppercase transition-all border ${day === 'today' ? 'bg-accent/10 border-accent/20 text-accent' : 'bg-surface border-border-subtle text-text-secondary'}`}
        >
          Today
        </button>
        <button 
          onClick={() => setDay('tomorrow')} 
          className={`flex-1 py-1.5 rounded-lg text-[12px] font-bold tracking-wider uppercase transition-all border ${day === 'tomorrow' ? 'bg-accent/10 border-accent/20 text-accent' : 'bg-surface border-border-subtle text-text-secondary'}`}
        >
          Tomorrow
        </button>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {loading ? (
          ...
          <div className="bg-surface border border-border-subtle p-6 rounded-2xl text-center">
            <p className="text-[13px] text-text-secondary">No trains scheduled for this route on this date.</p>
          </div>
        ) : (
          trains.map((train, index) => {
            // Mapping different possible property names from the API response
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {duration && (
                        <span className="text-[10px] font-medium text-text-secondary">{duration} trip</span>
                      )}
                    </div>
                  </div>
                </div>
